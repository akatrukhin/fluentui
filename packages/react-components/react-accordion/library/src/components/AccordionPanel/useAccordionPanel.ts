...
import { AccordionCollapseMotion } from './AccordionMotion';
...
export const useAccordionPanel_unstable = (
  ...
  return {
    ...
    components: {
      ...
      collapseMotion: AccordionCollapseMotion as React.FC<PresenceMotionSlotProps>,
    },
    root: slot.always(
      getIntrinsicElementProps('div', {
        // FIXME:
        // `ref` is wrongly assigned to be `HTMLElement` instead of `HTMLDivElement`
        // but since it would be a breaking change to fix it, we are caspackages/react-components/react-dialog/library/src/components/DialogSurfaceMotion.tsting ref to it's proper type
    ...
    collapseMotion: presenceMotionSlot(props.collapseMotion, {
      elementType: AccordionCollapseMotion,
      defaultProps: {
        appear: true,
        visible: open,
        unmountOnExit: true,
